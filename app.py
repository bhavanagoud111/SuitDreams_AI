...
# GitHub: github.com/bhavanagoud111
...
# Header
st.markdown('<h1>🏨 SuiteDreams AI</h1>', unsafe_allow_html=True)
st.markdown("**Discover the perfect stays — AI-powered hotel search and comprehensive filtering**")
...
with st.sidebar:
    ...
    st.markdown("**SuiteDreams AI** · Built with ❤️ by Bhavana Ediga")
...
async def run_hotel_agent(message: str, search_params: Dict[str, Any] = None) -> str:
    """
    Run the SuiteDreams AI agent with enhanced error handling and logging.
    ...
    try:
            ...
            async with ClientSession(read, write) as session:
                ...
                agent = Agent(
                    ...
                    instructions=dedent(f"""\
                        You are SuiteDreams AI, an advanced hotel assistant powered by comprehensive Airbnb data through MCP tools.
        ...
        return "⏰ **Timeout Error**: SuiteDreams AI search took too long. Please try again with a more specific query or increase the timeout in settings."
        ...
        if "API rate limit" in error_msg.lower():
            ...
            return "🌐 **Network Error**: SuiteDreams AI could not reach hotel data services. Please check your internet connection."
        else:
...
if execute_search:
    if not api_key:
        ...
        with st.spinner(f"🔍 SuiteDreams AI — running {search_mode.lower()}... This may take a moment."):
            try:
                # Show progress
                progress_bar = st.progress(0)
                status_text = st.empty()
                
                status_text.text("Initializing SuiteDreams AI...")
...
if 'search_results' in st.session_state:
    st.markdown("---")
    st.markdown("### 📋 SuiteDreams AI — Results")
    ...
    if export_results:
        ...
        st.download_button(
            ...
            file_name=f"suitedreams_ai_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
